...
function Dashboard() {
  ...
  useEffect(() => {
    const ok = window.localStorage.getItem("cef-admin") === "ok";
    setAllowed(ok);
  ...
  if (!allowed) {
    return (
      <div className="min-h-screen p-6">
        <div className="mx-auto max-w-md aero-window p-5 text-center">
          <h1 className="text-xl font-bold text-[oklch(0.25_0.12_250)]">Acceso requerido</h1>
          <p className="mt-2 text-sm">Ingresa desde la consulta publica con usuario y clave.</p>
          <a href="/" className="aero-btn mt-4 inline-block px-4 py-2 text-sm font-semibold">
            Volver a consulta
          </a>
        </div>
      </div>
