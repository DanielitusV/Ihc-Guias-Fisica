...
public class CashClosureRepository {
    ...
    public long save(
        ...
        try (var statement = connection.prepareStatement(
            ...
            statement.setString(8, closure.getBusinessDate().toString());
            statement.setString(9, closure.getCreatedAt().toString());
            statement.setLong(10, AcademicTermRepository.requireActiveId(connection));
    ...
    public List<CashClosure> findAll(Connection connection) throws Exception {
        String sql = """
                ...
                FROM cash_closures
                WHERE academic_term_id = (SELECT id FROM academic_terms WHERE status = 'OPEN')
                ORDER BY created_at ASC, id ASC
                """;
        return queryClosures(connection, sql, null, null);
    }

    public List<CashClosure> findAllByTerm(long academicTermId) throws Exception {
        String sql = """
                SELECT id, expected_cash, counted_cash, expected_qr,
                       reported_qr, notes, status, cancellation_reason, closure_date, created_at
                FROM cash_closures WHERE academic_term_id = ?
                ORDER BY created_at ASC, id ASC
                """;
        try (Connection connection = database.getConnection();
             var statement = connection.prepareStatement(sql)) {
            statement.setLong(1, academicTermId);
            List<CashClosure> values = new ArrayList<>();
            try (var result = statement.executeQuery()) {
                while (result.next()) values.add(mapClosure(result));
            }
            return values;
        }
    }
    ...
    public void update(
        ...
        try (var statement =
            ...
            if (statement.executeUpdate() != 1) {
                throw new IllegalStateException("No se puede modificar un cierre histórico");
            }
        }
    }
    ...
    private CashClosure mapClosure(
        ...
        CashClosure closure = new CashClosure(
                ...
                LocalDate.parse(result.getString("closure_date")),
