...
class CashClosureRepositoryTest {
    ...
    @Test
    void usesBusinessDateForRetroactiveClosure() throws Exception {
        CashClosureRepository repository = new CashClosureRepository(database);
        CashClosure closure = new CashClosure(
                ...
                LocalDateTime.of(2026, 8, 15, 8, 30)
        );

        long id = repository.save(closure, List.of(
                new CashClosureItem(0, 0, guideId, 10, 10)
        ));

        assertEquals(id, repository.findValidByDate(LocalDate.of(2026, 8, 14)).getId());
        assertNull(repository.findValidByDate(LocalDate.of(2026, 8, 15)));
        assertEquals(LocalDate.of(2026, 8, 14), repository.findById(id).getBusinessDate());
        assertEquals(LocalDateTime.of(2026, 8, 15, 8, 30), repository.findById(id).getCreatedAt());
    }
