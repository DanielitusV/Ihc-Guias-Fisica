...
\graphicspath{{images/prototipos/}}
...
La estructura del contenido se organizó según las tareas reales del proceso de distribución presencial. Se priorizaron las acciones frecuentes en el panel principal y se separaron las tareas complejas en pantallas específicas.

\begin{center}
\begin{tabularx}{\textwidth}{|B{4cm}|Y|}
\hline
Dashboard & Vista inicial del encargado. Incluye venta rápida por guía, stock actual, últimos registros, resumen de cuentas, ventas de la semana, salida de dinero y llegada de guías. \\
\hline
...
\hline
Consulta estudiante & Vista informativa para compradores. Presenta guía, materia, carreras relacionadas, stock, precio, horario, lugar y formas de pago. \\
\hline
\end{tabularx}
\end{center}
...
El mapa del sistema se definió con una navegación lateral permanente para que el encargado pueda moverse entre las secciones sin perder orientación. El Dashboard funciona como punto de entrada y resumen operativo.

\begin{center}
\begin{tabularx}{0.95\textwidth}{|>{\centering\arraybackslash}X|}
\hline
Dashboard $\rightarrow$ Registrar venta $\rightarrow$ Dashboard \\
\hline
Dashboard $\rightarrow$ Inventario $\rightarrow$ Cierre de caja \\
\hline
Dashboard $\rightarrow$ Cuentas $\rightarrow$ Cierre de caja \\
\hline
Dashboard $\rightarrow$ Pedidos $\rightarrow$ Inventario \\
\hline
Dashboard $\rightarrow$ Consulta estudiante $\rightarrow$ Dashboard \\
\hline
\end{tabularx}
\end{center}

Esta estructura responde a la necesidad de reducir cambios innecesarios de pantalla. Las tareas más frecuentes se ubican cerca del Dashboard, mientras que las tareas de verificación, como el cierre de caja, se mantienen separadas para evitar confusiones.
...
Este flujo responde a la necesidad detectada en los compradores, quienes quieren saber si la guía está disponible antes de acercarse al Centro.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_dashboard.png}
    \caption{Wireframe del Dashboard: resumen operativo, ventas rápidas, cuentas e historial.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_venta.png}
    \caption{Wireframe de registro de venta: selección de guía, cantidad y método de pago.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_inventario.png}
    \caption{Wireframe de inventario: guías fiadas, compradas, vendidas, stock restante y estado.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_cuentas.png}
    \caption{Wireframe de cuentas: separación entre caja física, cuenta QR y cuenta del encargado.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_cierre.png}
    \caption{Wireframe de cierre de caja: verificación de guías, dinero físico y pagos QR.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_pedidos.png}
    \caption{Wireframe de pedidos: llegadas de guías, costos, estado de pago y deuda.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth,height=0.58\textheight,keepaspectratio]{prototipo_consulta.png}
